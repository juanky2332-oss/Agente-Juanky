...
export const POST = manejar(async (req: Request) => {
  const b = (await req.json()) as { ingreso: string; importe?: number | string; fecha?: string; metodo?: string; notas?: string };
...
export const PATCH = manejar(async (req: Request) => {
  const b = (await req.json()) as { id: string; importe?: number | string; fecha?: string; metodo?: string; notas?: string };
