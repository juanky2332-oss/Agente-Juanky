...
import { CATEGORIAS, clasificar } from "@/lib/finanzas";
import { num, fechaISO } from "@/lib/parse";
...
export const POST = manejar(async (req: Request) => {
  const { base64, mime } = (await req.json()) as { base64: string; mime: string };
  if (!base64) throw new ErrorN8n("Falta el archivo", 400);
